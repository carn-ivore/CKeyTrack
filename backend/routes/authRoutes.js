...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            }); // This is what appears on Postman
        } else {
        ...
        res.status(500).json({
            message: "Internal Server Error authRoutes:33",
        });
    }
});
...
async function getAuthorizedKeys(employee_id) {
    try {
        ...
        const authorizedKeys = rows
            .filter(
                (row) =>
                    row[1] === employee_id.toString() &&
                    new Date(row[3]) > new Date()
            ) // Filter by employee_id and check authorization is not expired
