...
router.post("/", async (req, res) => {
    ...
    if (!employee_id) {
        return res
            .status(400)
            .json({ message: "availkeysroutes:16 Employee ID is required" });
    }
    ...
    try {
        ...
        if (employee_id) {
            ...
            console.log(
                "availableKeysRoutes:41 Authorized keys for employee_id:",
                authorizedKeys
            );
        ...
        res.status(500).json({
            message: "Internal Server Error availableKeysRoutes:43",
        });
    }
});

// Function to get available keys based on authorized keys
async function getAvailableKeys(authorizedKeys, sheets) {
    // Accepts sheets as an argument
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId: SPREADSHEET_ID,
        ...
        const checkedOutKeys = rows
            .filter((row) => row[4] === "" && !row[2]) // Checks if key_id exists and that checkedin_timestamp is empty
            .map((row) => parseInt(row[2])) // Extracts the key_id of checked-out keys
            .filter((key_id) => !isNaN(key_id)); // Filter out NaN values
        ...
        const availableKeys = authorizedKeys.filter(
            (key) => !checkedOutKeys.includes(key)
        );
